    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    pitchButton.setButtonText("Pitch shift");
    pitchButton.addListener(this);
    addAndMakeVisible(pitchButton);
    //    pitchButton.onClick = [this] { updateToggleState(&pitchButton, "Pitch shift"); };
...
void TestTake2AudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    pitchButton.setBounds(200, getHeight() / 2 - 100, 100, 50);
}
...
void TestTake2AudioProcessorEditor::buttonClicked(juce::Button *button) {
    if (button == &pitchButton) {
        if (audioProcessor.pitchShift == false ) audioProcessor.pitchShift = true;
        else audioProcessor.pitchShift = false;
    }
}